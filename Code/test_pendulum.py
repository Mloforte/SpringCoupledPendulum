...
class Pendulum:
    ...
    def step(self, t, surface):
        ...
        # self.position1 = [self.x1[-1], self.y1[-1]]  # Last position of pendulum 1
        # self.position2 = [self.x2[-1], self.y2[-1]]  # Last position of pendulum 2
        ...
        pygame.draw.circle(surface, self.color, (int(self.position1[0]), int(self.position1[1])), 15)
        pygame.draw.circle(surface, self.color, (int(self.position2[0]), int(self.position2[1])), 15)
    ...
    def G_adim(self, y, t):
        ...
        d_theta1 = dtheta1
        dd_theta1 = self.A*np.sin(theta1) - self.B*np.sin(theta1 - theta2) * \
            (1 - (self.l / np.sqrt(self.l**2 + 2*self.l**2*(1 - np.cos(theta1 + theta2)))))

        d_theta2 = dtheta2

        dd_theta2 = self.A*np.sin(theta2) - self.C*np.sin(theta2 - theta1) * \
            (1 - (self.l / np.sqrt(self.l**2 + 2*self.l**2*(1 - np.cos(theta1 + theta2)))))
